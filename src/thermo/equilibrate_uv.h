...
DISPATCH_MACRO bool partition_uv_state(
  ...
  for (int reaction_index = 0; reaction_index < nreaction; ++reaction_index) {
    ...
    T log_saturation =
        eval_logsvp(svp_kind[reaction_index],
  ...
  while (iter++ < *max_iter) {
    ...
    while (first < last) {
      ...
      for (int i = 0; i < nspecies; i++) {
        ...
        if (nu > 0) {  // reactant
          if (conc[i] / nu < limiting_reactant)
            limiting_reactant = conc[i] / nu;
          if (conc[i] == 0.) {
            nu_absent += nu;
            log_nu += nu * log(nu);
          } else {
    ...
    err_code = leastsq_kkt<T, Backend>(rhs, gain, stoich_active, conc,
