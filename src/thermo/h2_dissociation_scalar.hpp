...
// tensor ops, and on small blocks their cost is dominated by per-op dispatch
// overhead that does not scale with the field size. The fused kernels turn the
// loop inside out -- one launch per solve, per-cell Newton -- and need the
// physics as ONE scalar function body. This header is that body:
// cp_R_of/h_R_of/s_R_of/speciate/eval, line-for-line the same math as
// h2_dissociation.hpp, in double. NO torch, NO printf, header-only, host (CPU)
// only.
//
// SINGLE SOURCE OF TRUTH for the NASA-9 coefficients: the caller passes `ab`,
// the SAME (2,3,9) = [low|high][H2,H,He][coeff] block that
// nasa9_coeffs_by_name({"H2","H","He"}) builds for the torch path. We never
// hardcode the 54 numbers.
...
// on a (T,c) grid (rtol 1e-12, atol 1e-9). Do NOT change the semantics here
// without re-running it -- it is the transcription guard.
...
//! log(T) five times per call. Bit-identical: same std::log value, just
//! computed once.
...
//! c (the c cancels: n_i proportional to cc, U divides by cc). The 5-argument
//! eval() below still computes it via speciate(kTref,cc), as the torch path
//! does; the fused Newton loops hoist THIS constant out of the iteration
//! instead.
